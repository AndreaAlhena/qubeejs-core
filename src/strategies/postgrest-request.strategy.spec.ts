...
describe('PostgrestRequestStrategy', () => {
  ...
  describe('operator filters', () => {
    ...
    it('should emit ILIKE as col=ilike.pattern', () => {
      const state = {
        ...baseState,
        operatorFilters: [
          { field: 'name', operator: FilterOperatorEnum.ILIKE, values: ['%john%'] },
        ],
      };
      const uri = strategy.buildUri(state, options);
    ...
    it('should emit NOT for a single value as col=not.eq.val', () => {
      const state = {
        ...baseState,
        operatorFilters: [
          { field: 'status', operator: FilterOperatorEnum.NOT, values: ['deleted'] },
        ],
      };
      const uri = strategy.buildUri(state, options);
    ...
    it('should emit NULL(true) as col=is.null', () => {
      const state = {
        ...baseState,
        operatorFilters: [
          { field: 'deletedAt', operator: FilterOperatorEnum.NULL, values: [true] },
        ],
      };
      const uri = strategy.buildUri(state, options);
    ...
    it('should emit NULL(false) as col=is.not.null', () => {
      const state = {
        ...baseState,
        operatorFilters: [
          { field: 'deletedAt', operator: FilterOperatorEnum.NULL, values: [false] },
        ],
      };
      const uri = strategy.buildUri(state, options);
        ...
        operatorFilters: [
          {
            ...
            values: ['true' as unknown as boolean],
          },
        ],
      };
    ...
    it('should throw InvalidFilterOperatorValueError for NULL with wrong arity', () => {
      const state = {
        ...baseState,
        operatorFilters: [
          { field: 'deletedAt', operator: FilterOperatorEnum.NULL, values: [true, false] },
        ],
      };
    ...
    it('should emit CONTAINS as col=ilike.%val%', () => {
      const state = {
        ...baseState,
        operatorFilters: [
          { field: 'name', operator: FilterOperatorEnum.CONTAINS, values: ['john'] },
        ],
      };
      const uri = strategy.buildUri(state, options);
  ...
  describe('embedded resources', () => {
    ...
    it('should emit one select param holding multiple embedded relations', () => {
      ...
      expect(uri.match(/select=/g)).toHaveLength(1);
    });
